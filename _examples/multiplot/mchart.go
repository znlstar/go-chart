...
import "os"
import "time"
import chart "github.com/znlstar/go-chart"

import "github.com/znlstar/go-chart/drawing"

func main() {
	c1 := legend_left()
	c2 := descending()
	c3 := stock_analysis()
	layout, err := chart.NewLayout(3, 1, -1, -1)
	mcs := &chart.MChart{
		Charts:       []chart.Chart{c2, c3, c1},
		CanvasLayout: layout,
	}
	_ = c1
	_ = c2
	_ = c3
	fp, err := os.Create("1.png")
	if err != nil {
		fmt.Println(err)
		return
	}
	err = mcs.Render(chart.PNG, fp)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ParseTime() []time.Time {
	var t []time.Time = make([]time.Time, 0)
	data := []string{"2017-05-03 20:00:01", "2017-05-03 20:01:01", "2017-05-03 20:02:01", "2017-05-03 20:03:01", "2017-05-03 20:04:01", "2017-05-03 20:05:01"}
	loc, _ := time.LoadLocation("Local")
	for _, v := range data {
		vt, err := time.ParseInLocation("2006-01-02 15:04:05", v, loc)
		if err != nil {
			continue
		}
		t = append(t, vt)
...
func descending() (graph chart.Chart) {
	graph = chart.Chart{
		XAxis: chart.XAxis{
			Style: chart.Style{
				Show: true,
...
func legend_left() (graph chart.Chart) {
	graph = chart.Chart{
		XAxis: chart.XAxis{
...
func stock_analysis() chart.Chart {
